...
<head>
    ...
    <title>History for <?= htmlspecialchars($monitor['name'] ?? 'Monitor') ?></title>
...
<div class="container mx-auto p-4 md:p-8">
    <header class="mb-8">
        ...
        <?php if ($monitor): ?>
        <h1 class="text-4xl font-bold mt-2">Ping History: <?= htmlspecialchars($monitor['name']) ?></h1>
        <p class="text-gray-600 mt-1">Showing last 200 checks for <?= htmlspecialchars($monitor['ip_address']) ?></p>
        <?php else: ?>
        <h1 class="text-4xl font-bold mt-2">Monitor Not Found</h1>
        <?php endif; ?>
    </header>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Check Time</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if (empty($history)): ?>
                        <tr><td colspan="2" class="px-6 py-4 text-center text-gray-500">No history found.</td></tr>
                    <?php else: foreach ($history as $record): ?>
                        <tr>
                            <td class="px-6 py-4">
